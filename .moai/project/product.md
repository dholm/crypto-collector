...
- Recomputed on the periodic collector tick from the persisted `1d` (daily) OHLCV history in `coin_candles`. A discovery route at `GET /v1/coins/{coin_id}/cycle-projection` (base path, no model param) returns available models and their properties. Keyset-paginated data route at `GET /v1/coins/{coin_id}/cycle-projection/{model}` where `{model}` ∈ {`replay`, `composite`} with optional `?cycle=N`, `?vs_currency`, `?as_of=<RFC3339>`, and pagination params. The `replay` model serves raw historical replay; the `composite` model adds power-law + phase + mean-reversion projection with P10/P90 confidence bands. The `as_of` parameter enables point-in-time historical reads per model, reconstructing the projection as it would have appeared at a given timestamp—useful for comparing the projection that was active at that date against the price actuals that followed. Known halving dates treated as block-derived constants; future halvings extend the model via code updates.
